...
public class StepsView extends LinearLayout implements StepsViewIndicator.OnDrawListener {
  private static final String TAG = StepsView.class.getSimpleName();
  ...
  private void init() {
    ...
    mLayoutContents = (LinearLayout) rootView.findViewById(R.id.layout_contents);
  }
  ...
  public StepsView setTypeface(Typeface typeface) {
    mTypeface = typeface;
    return this;
  }
  ...
  public StepsView setCompletedPosition(int completedPosition) {
    Log.i(TAG, "setCompletedPosition: mCompletedItem = " + completedPosition);
  ...
  public void drawView() {
    ...
    if (mCompletedPosition < 0 || mCompletedPosition > mLabels.length) {
...
//    List<Float> indicatorPosition = mStepsViewIndicator.getThumbContainerXPosition();
    ...
    if (mLabels != null) {
      for (int i = 0; i < mLabels.length; i++) {
        ...
        textView.setTextColor(i == mCompletedPosition - 1 ? mProgressColorIndicator : mBarColorIndicator);
        ...
        mLayoutContents.addView(textView);
      }
    }
  }
}
